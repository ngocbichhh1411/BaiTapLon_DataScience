...
if load_success:
    ...
    with col1:
        ...
        # Tự động trích xuất danh sách lựa chọn từ chính bộ mã hóa của ông để tránh bị gõ sai chữ
        list_make = sorted(list(encoders['Make'].keys()))
        list_model = sorted(list(encoders['Model'].keys()))
        ...
        # Tạo các hộp chọn (Dropdown menu)
        make = st.selectbox("Hãng xe (Make):", list_make)
        model_car = st.selectbox("Dòng xe (Model):", list_model)
